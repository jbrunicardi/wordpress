...
if [[ "$1" == apache2* ]] || [ "$1" == php-fpm ]; then
	...
	for i in wp-config*;do sed -r 's/\r\n|\r/\n/g' $i > /tmp/$i; mv /tmp/$i $i; done
...
EOPHP
	...
	set_config() {
		...
		sed -r "s/($start\s*).*($end)$/\1$(sed_escape_rhs "$(php_escape "$value" "$var_type")")\3/" wp-config.php > /tmp/wp-config.php
